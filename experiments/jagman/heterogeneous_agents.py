...
time_iteration_direct = improved_time_iteration  # compat
...
def stat_dist(model, dr, Nf, itmaxL=5000, tolL=1e-8, verbose=False):
    ...
    for i_s in range(1, Nend):
        ...
        Qtmp = single_state_transition_matrix(
            sgrid, sprimef[:, Nexo + i_s], Nf, Nf[Nexo + i_s]
        ).toarray()
...
def single_state_transition_matrix(grid, vals, Nf, Nstate):
    ...
    QstateL = spa.coo_matrix(
        (weighttolower, (idxlist, idL.flatten())), shape=(Ntot, Nstate)
    )
    QstateU = spa.coo_matrix(
        (weighttoupper, (idxlist, idU.flatten())), shape=(Ntot, Nstate)
    )
...
def set_solve_stat(model, Nf, aggvarname, Aval, verbose=False):
    ...
    if "direct_response" in model.equations:
        dr = time_iteration_direct(model, verbose=verbose).dr
    else:
        dr = time_iteration(model, verbose=verbose).dr
...
def supply_demand(
    model, varname, aggvarname, pricename, Nf, bounds, numpoints=20, verbose=True
):
    """
    ...
    for i in range(numpoints):
        ...
        # Get the price
        p[i] = model.calibration[pricename]
...
def dr_to_sprime(model, dr, Nf):
    ...
    N = drc.shape[0]
    ee = np.zeros([N, Nexo])
    sprimef = trans(ee, gridf, drc, ee, parms)
...
def fine_grid(model, Nf):
    ...
    gridf = np.hstack(
        [np.repeat(mgrid, sgrid.shape[0], axis=0), np.tile(sgrid, (mgrid.shape[0], 1))]
    )
...
def exog_grid_trans(model, Nf):
    ...
    if not np.all(diags == 0):
        raise Exception(
            "Covariance matrix is not diagonal. Cannot proceed assuming that AR(1)s are indpendent."
        )
    ...
    sss, xss, ess, pss = model.calibration[
        "states", "controls", "exogenous", "parameters"
    ]
    diff = trans(ess, sss, xss, ess, pss, diff=True)
